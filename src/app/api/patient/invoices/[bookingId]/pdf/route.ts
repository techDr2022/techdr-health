...
export async function GET(
  ...
  try {
    ...
    if (!isPatient && session.user.role !== "ADMIN") {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    const key = booking.invoicepdfkey.replace(/^\/+/, "");
    ...
    const filename = booking.invoicenumber || `invoice-${bookingId}`;
    ...
    console.error("[patient/invoices/pdf]", error);
    return NextResponse.json({ error: "Unable to fetch invoice PDF." }, { status: 500 });
  }
}
