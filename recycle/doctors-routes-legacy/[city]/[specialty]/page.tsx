...
type Props = { params: { city: string; specialty: string } };
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  if (!isValidCitySpecialtyRoute(params.city, params.specialty)) {
  ...
  return getCitySpecialtyPageSEO(cityName, specialty.name, doctors.length);
}

export default async function CitySpecialtyDoctorsPage({ params }: Props) {
  const citySlug = resolveCitySlug(params.city);
  const specialty = getSpecialtyBySlug(params.specialty);
  if (!citySlug || !specialty) notFound();

  const cityName = formatCityName(citySlug);
  const allDoctors = await getCachedLiveDoctorCatalog();
  const doctors = allDoctors.filter((doctor) => doctor.specialtySlug === params.specialty);
  ...
  const pagePath = buildCitySpecialtyPath(citySlug, params.specialty);
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <section className="mt-10">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {listSpecialtySlugs()
                .filter((slug) => slug !== params.specialty)
                .slice(0, 12)
                .map((slug) => {
                  ...
                  return (
                    <Link
                      key={slug}
                      href={buildCitySpecialtyPath(citySlug, slug)}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {CITY_TARGETS.filter((otherCity) => otherCity !== citySlug)
                ...
                .map((otherCity) => (
                  <Link
                    key={otherCity}
                    href={buildCitySpecialtyPath(otherCity, params.specialty)}
